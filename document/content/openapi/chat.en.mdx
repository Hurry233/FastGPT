---
title: Chat API
description: GPTW Agent OpenAPI Chat Interface
---
...
The `v1` chat API is compatible with the `GPT` interface! If you're using the standard `GPT` official API, you can access GPTW Agent by simply changing the `BaseUrl` and `Authorization`. However, note these rules:
...
- responseChatItemId: string | undefined 。如果传入，则会将该值作为本次对话的响应消息的 ID，GPTW Agent会自动将该 ID 存入数据库。请确保，在当前`chatId`下，`responseChatItemId`是唯一的。
