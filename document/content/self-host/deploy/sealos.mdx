---
title: Sealos 部署
description: 使用 Sealos 一键部署 GPTW Agent
---
...
GPTW Agent 使用了 one-api 项目来管理模型池，其可以兼容 OpenAI 、Azure 、国内主流模型和本地模型等。
...
GPTW Agent 商业版共包含了2个应用（fastgpt, fastgpt-plus）和2个数据库，使用多 Api Key 时候需要安装 OneAPI（一个应用和一个数据库），总计3个应用和3个数据库。
...
在 Sealos 中，你可以打开`应用管理`（App Launchpad）看到部署的 GPTW Agent，可以打开`数据库`（Database）看到对应的数据库。

在`应用管理`中，选中 GPTW Agent，点击变更，可以看到对应的环境变量和配置文件。

![](../../../public/imgs/fastgptonsealos1.png)

<Alert icon="🤖" context="success">
  在 Sealos 上，GPTW Agent 一共运行了 1 个服务和 2
