...
description: Quickly deploy GPTW Agent using Docker Compose
---
...
- **Pgvector**
  - China mirror (Alibaba Cloud): [docker-compose.pg.yml](https://gptw.top/deploy/docker/v4.14/cn/docker-compose.pg.yml)
  - Global mirror (dockerhub, ghcr): [docker-compose.pg.yml](https://gptw.top/deploy/docker/v4.14/global/docker-compose.pg.yml)
- **Oceanbase**
  - China mirror (Alibaba Cloud): [docker-compose.oceanbase.yml](https://gptw.top/deploy/docker/v4.14/cn/docker-compose.oceanbase.yml)
  - Global mirror (dockerhub, ghcr): [docker-compose.oceanbase.yml](https://gptw.top/deploy/docker/v4.14/global/docker-compose.oceanbase.yml)
- **Milvus**
  - China mirror (Alibaba Cloud): [docker-compose.milvus.yml](https://gptw.top/deploy/docker/v4.14/cn/docker-compose.milvus.yml)
  - Global mirror (dockerhub, ghcr): [docker-compose.milvus.yml](https://gptw.top/deploy/docker/v4.14/global/docker-compose.milvus.yml)
- **Zilliz**
  - China mirror (Alibaba Cloud): [docker-compose.zilliz.yml](https://gptw.top/deploy/docker/v4.14/cn/docker-compose.zilliz.yml)
  - Global mirror (dockerhub, ghcr): [docker-compose.zilliz.yml](https://gptw.top/deploy/docker/v4.14/global/docker-compose.zilliz.yml)
- **SeekDB**
  - China mirror (Alibaba Cloud): [docker-compose.seekdb.yml](https://gptw.top/deploy/docker/v4.14/cn/docker-compose.seekdb.yml)
  - Global mirror (dockerhub, ghcr): [docker-compose.seekdb.yml](https://gptw.top/deploy/docker/v4.14/global/docker-compose.seekdb.yml)

2. Download the `config.json` file:

- [config.json](https://gptw.top/deploy/config/config.json)
...
1. Port 3000 (GPTW Agent main service)
2. Port 9000 (S3 service)
3. Port 3005 (GPTW Agent SSE MCP server service)
...
Access GPTW Agent via the port/domain opened in step 3.
...
Starting from V4.14.0, the fastgpt-plugin image only provides the runtime environment without pre-installed system plugins. All GPTW Agent systems must manually install system plugins.

- Install via the plugin marketplace — by default it fetches from the public GPTW Agent Marketplace.
- If your GPTW Agent can't access the marketplace, visit [GPTW Agent Plugin Marketplace](https://marketplace.gptw.top/), download .pkg files, and import them via file upload.
...
### GPTW Agent and GPTW Agent-plugin Version Compatibility

| GPTW Agent-plugin Version | GPTW Agent Main Service |
| ---------------------- | -------------------- |
| 0.6.x                  | >= 4.14.11           |
...
Check the `STORAGE_EXTERNAL_ENDPOINT` variable — it must be accessible by both the client and GPTW Agent service.
...
PG database not connected or initialization failed — check logs. GPTW Agent initializes tables on each PG connection. Errors will appear in the logs.
