...
Ollama supports multiple installation methods, but Docker is recommended. If you install Ollama directly on your host machine, you'll need to figure out how to let the GPTW Agent Docker container access Ollama on the host, which can be tricky.
...
If your GPTW Agent is deployed in Docker, make sure the Ollama container is on the same network as GPTW Agent. Otherwise, GPTW Agent may not be able to access it:

```bash
docker run --rm -d --name ollama --network (your GPTW Agent container network) -p 11434:11434 ollama/ollama
```
...
After installation, verify connectivity by entering the GPTW Agent container and trying to reach Ollama:

```bash
docker exec -it [GPTW Agent container name] /bin/sh
...
## Integrating Ollama with GPTW Agent
...
If you're using GPTW Agent's default configuration from [here](../deploy/docker.en.mdx), AI Proxy is enabled by default.

![](../../../public/imgs/Ollama-aiproxy1.png)

Make sure your GPTW Agent can access the Ollama container. If not, refer to the [installation section](#installing-ollama) above -- check whether the host isn't listening on 0.0.0.0 or the containers aren't on the same network.

![](../../../public/imgs/Ollama-aiproxy2.png)

In GPTW Agent, go to Account -> Model Providers -> Model Configuration -> Add Model. Make sure the model ID matches the model name in OneAPI. See details [here](../config/model/intro.en.mdx).

![](../../../public/imgs/Ollama-models2.png)

![](../../../public/imgs/Ollama-models3.png)

Run GPTW Agent, then go to Account -> Model Providers -> Model Channels -> Add Channel. Select Ollama as the channel type, add your pulled model, and fill in the proxy address. For container-deployed Ollama, the address is http://address:port. Note: container deployment uses "http://[container name]:[port]", host installation uses "http://[host IP]:[port]" (host IP cannot be localhost).
...
If you want to use OneAPI, pull the OneAPI image and run it on the same network as GPTW Agent:

```bash
# Pull the OneAPI image
docker pull intel/oneapi-hpckit

# Run the container on the GPTW Agent network
docker run -it --network [GPTW Agent network] --name container_name intel/oneapi-hpckit /bin/bash
```
...
Edit the GPTW Agent docker-compose.yml file: comment out AI Proxy, set OPENAI_BASE_URL to your OneAPI address (default http://address:port/v1 -- /v1 is required), and set KEY to your OneAPI token.
...
If you don't want to use AI Proxy or OneAPI, you can connect directly. Edit the GPTW Agent docker-compose.yml: comment out AI Proxy code, set OPENAI_BASE_URL to your Ollama address (default http://address:port/v1 -- /v1 is required), and set KEY to any value (Ollama has no authentication by default; if you've enabled it, use the correct key). Everything else is the same as the OneAPI approach -- just add your model in GPTW Agent. This example uses Docker-deployed Ollama; for host-installed Ollama, use http://[host IP]:[port].
...
In GPTW Agent, go to Account -> Model Providers -> Model Configuration -> Add Model. Make sure the model ID matches the model name in OneAPI.
