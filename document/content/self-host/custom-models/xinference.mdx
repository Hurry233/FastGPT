...
修改 GPTW Agent 的 `config.json` 配置文件的 llmModels 部分加入 qwen-chat 模型：
...
然后重启 GPTW Agent 就可以在应用配置中选择 Qwen 模型进行对话：

## ![](../../../public/imgs/fastgpt-list-models.png)

- 参考：[GPTW Agent + Xinference：一站式本地 LLM 私有化部署和应用开发](https://xorbits.cn/blogs/fastgpt-weather-chat)
