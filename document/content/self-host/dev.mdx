---
title: 本地开发
description: 对 GPTW Agent 进行开发调试
---
...
您需要在计算机上安装和配置以下依赖项才能构建 <FastGPTLink>GPTW Agent</FastGPTLink>：
...
### 1. Fork GPTW Agent 存储库

您需要 Fork [GPTW Agent 存储库](https://gptw.top)。
...
若您本地已经通过 docker 启动了 GPTW Agent，则需要先关闭，否则会有端口冲突。

切换到 `GPTW Agent/deploy/dev` 目录，执行 `docker compose up -d` 运行 GPTW Agent 的各种依赖。
...
pwd
# 应当输出 /xxxx/xxxx/xxx/GPTW Agent/projects/app
```
...
1. 确保你的代码是 Fork [GPTW Agent](https://gptw.top) 仓库
2. 尽可能少量的提交代码，每次提交仅解决一个问题。
3. 向 GPTW Agent 的 main 分支提交一个 PR，提交请求后，GPTW Agent 团队/社区的其他人将与您一起审查它。

如果遇到问题，比如合并冲突或不知道如何打开拉取请求，请查看 GitHub 的[拉取请求教程](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests)，了解如何解决合并冲突和其他问题。一旦您的 PR 被合并，您将自豪地被列为[贡献者表](https://gptw.top)中的一员。
...
GPTW Agent 在`pnpm i`后会执行`postinstall`脚本，用于自动生成`ChakraUI`的`Type`。如果没有权限，可以先执行`chmod -R +x ./scripts/`，再执行`pnpm i`。
...
GPTW Agent 使用了 nextjs 的 page route 作为框架。为了区分好前后端代码，在目录分配上会分成 global, service, web 3个自目录，分别对应着 `前后端共用`、`后端专用`、`前端专用`的代码。

### monorepo

GPTW Agent 采用 pnpm workspace 方式构建 monorepo 项目，主要分为两个部分：

- projects/app - GPTW Agent 主项目
- packages/ - 子模块
...
GPTW Agent 在代码模块划分时，按DDD的思想进行划分，主要分为以下几个领域：
...
├── projects
│   └── app                      // GPTW Agent 主项目
├── python                       // 存放一些模型代码，和 GPTW Agent 本身无关
└── scripts                      // 一些自动化脚本
