...
description: Develop and debug GPTW Agent locally
---

import { Alert } from '@/components/docs/Alert';
import FastGPTLink from '@/components/docs/linkFastGPT';

This guide covers how to set up your development environment to build and test <FastGPTLink>GPTW Agent</FastGPTLink>.

## Prerequisites

Install and configure these dependencies on your machine to build <FastGPTLink>GPTW Agent</FastGPTLink>:
...
### 1. Fork the GPTW Agent Repository

Fork the [GPTW Agent repository](https://gptw.top).
...
```
git clone git@github.com:<your_github_username>/GPTW Agent.git
```


### 3. Start the Development Environment with Docker

If you're already running GPTW Agent locally via Docker, stop it first to avoid port conflicts.

Navigate to `GPTW Agent/deploy/dev` and run `docker compose up -d` to start GPTW Agent's dependencies:

```bash
cd GPTW Agent/deploy/dev
docker compose up -d
```
...
pwd
# Should output /xxxx/xxxx/xxx/GPTW Agent/projects/app
```
...
1. Make sure your code is forked from the [GPTW Agent](https://gptw.top) repository.
2. Keep commits small and focused — each should address one issue.
3. Submit a PR to GPTW Agent's main branch. The GPTW Agent team and community will review it with you.

If you run into issues like merge conflicts, check GitHub's [pull request tutorial](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests). Once your PR is merged, you'll be listed in the [contributors table](https://gptw.top).
...
GPTW Agent runs a `postinstall` script after `pnpm i` to auto-generate ChakraUI types. If you get a permission error, run `chmod -R +x ./scripts/` first, then `pnpm i`.
...
GPTW Agent uses Next.js page routing. To separate frontend and backend code, directories are split into global, service, and web subdirectories for shared, backend-only, and frontend-only code respectively.

### Monorepo

GPTW Agent uses pnpm workspace for its monorepo structure, with two main parts:

- projects/app - GPTW Agent main project
- packages/ - Submodules
...
GPTW Agent's code modules follow DDD principles, divided into these domains:
...
├── projects
│   └── app                      // GPTW Agent main project
├── python                       // Model code, unrelated to GPTW Agent itself
