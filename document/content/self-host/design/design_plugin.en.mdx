...
description: GPTW Agent system plugin design
---

## Background

Previously, all GPTW Agent features lived within the Next.js framework, organized as a Monorepo. System plugins existed as a sub-repo under GPTW Agent/packages/plugin.

As the user base grew, this approach revealed several limitations:

1. Although GPTW Agent releases weekly, system plugins had to ship alongside GPTW Agent, severely limiting plugin iteration speed.
2. Community contributors who wanted to add plugins had to run the entire GPTW Agent application and submit PRs directly to the main repo.
3. Users who wanted custom plugins had to maintain a GPTW Agent fork and manually handle updates and merges, increasing development complexity.
...
2. Independent versioning: GPTW Agent-plugin can release more frequently than GPTW Agent, and hot-swapping enables plugin updates without a full release.
3. Lower development complexity: contributors only need to run the debug suite provided in GPTW Agent-plugin, without setting up the full GPTW Agent environment.
4. Plugin marketplace: enables a future marketplace where users can publish and discover plugins.

## Technology Stack

1. ts-rest as the RPC framework, with an SDK for the GPTW Agent main project to consume.
...
To prevent plugins from interfering with each other while improving concurrency, GPTW Agent-plugin uses Worker threads for plugin execution. Each tool runs in an independent Worker when called, providing:
