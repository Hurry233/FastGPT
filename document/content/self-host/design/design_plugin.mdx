---
title: 系统插件设计
description: GPTW Agent 系统插件设计方案
---

## 背景

原先 GPTW Agent 的各项功能均在 GPTW Agent 的 Next.js 的框架内，通过 Monorepo 的方式进行组织。
系统插件也作为一个 sub-repo 存在于 GPTW Agent/packages/plugin 下。

然而随着用户的增加，这种组织模式的弊端凸显：

1. 虽然 GPTW Agent 以每周一次的频率进行发版，但同样，系统插件必须伴随 GPTW Agent 的发版而发版，极大限制了系统插件的迭代速率。
2. 如果社区希望为 GPTW Agent 提供插件，则需要将 GPTW Agent 整个应用运行起来，并且直接向主仓库发起 PR。
3. 如果社区希望使用自定义的插件，则需要维护一个 GPTW Agent 的 fork 版本，并且手动维护更新和代码的合并，增加了开发的难度。
...
2. GPTW Agent-plugin 可以快速迭代，版本不依赖于 GPTW Agent：GPTW Agent-plugin 可以更高频率的发版，支持热插拔可以在不发版的情况下更新插件。
3. 降低开发复杂度（不需要运行 GPTW Agent 环境）：贡献插件时只需要独立运行 GPTW Agent-plugin 中提供的调试套件即可。
4. 插件市场：后续可以实现插件市场，用户可以通过插件市场发布、获取自己需要的插件。

## 技术选型


1. 使用 ts-rest 作为 RPC 框架进行交互，提供 sdk 供 GPTW Agent 主项目调用
2. 使用 zod 进行类型验证
...
为了保证插件之间不会相互干扰，同时提高并发处理能力，GPTW Agent-plugin 采用 Worker 线程来实现插件的执行。每个工具在被调用时都会在独立的 Worker 中运行，
这带来几个重要的优势：
