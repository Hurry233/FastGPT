...
description: GPTW Agent model configuration guide
---
...
GPTW Agent uses the `AI Proxy` service to connect to different model providers. AI Proxy also provides load balancing, model logging, and analytics dashboards to help you monitor model usage.
...
You can configure models from the `Account - Model Providers` page in GPTW Agent.
...
4. Model Mapping: Maps the model name in GPTW Agent requests to the actual model name at the provider. For example:
...
In GPTW Agent, the model is `gpt-4o-test`, and requests to AI Proxy also use `gpt-4o-test`. When AI Proxy forwards the request upstream, the actual `model` value becomes `gpt-4o`.
...
GPTW Agent provides simple tests for each model type on the UI to verify that models are working correctly. Each test sends an actual request using a template.
...
```json
{
  ...
  "metadata": {
    ...
    "provider": "OpenAI", // Model provider, used for categorization. Built-in providers: https://gptw.top You can submit a PR for new providers, or use "Other"
...
You can find model provider configuration files in the `modules/model/provider` directory of the `GPTW Agent-plugin` project and add model configurations there. Make sure the `model` field is unique across all models. For field descriptions, refer to [Model Configuration Fields](./intro.en.mdx#add-via-configuration-file).
