...
This guide covers environment variable configuration for object storage providers supported by GPTW Agent, including self-hosted MinIO, AWS S3, Alibaba Cloud OSS, and Tencent Cloud COS.
...
- `STORAGE_PUBLIC_BUCKET` GPTW Agent public resource bucket name
- `STORAGE_PRIVATE_BUCKET` GPTW Agent private resource bucket name

### Transfer Behavior

- Uploads always go through the GPTW Agent backend proxy.
...
- `STORAGE_S3_CDN_ENDPOINT` [Optional] CDN endpoint for S3/MinIO presigned download and preview access. GPTW Agent rewrites generated file access URLs to this CDN endpoint only when the current download uses `presigned` mode. This variable does not change the default download mode. Uploads still go through the GPTW Agent backend proxy and do not use the CDN.
