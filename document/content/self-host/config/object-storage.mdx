...
这里提供了 GPTW Agent 目前支持的对象存储厂商，包括自部署的 MinIO、AWS S3、阿里云 OSS 和腾讯云 COS 的环境变量配置说明
...
- `STORAGE_PUBLIC_BUCKET` GPTW Agent 公开资源存储桶桶名
- `STORAGE_PRIVATE_BUCKET` GPTW Agent 私有资源存储桶桶名

### 传输模式说明

- 上传固定走 GPTW Agent 后端代理。
...
- `STORAGE_S3_CDN_ENDPOINT`【可选】S3/MinIO 预签名下载与预览访问的 CDN 地址。仅在当前下载走 `presigned` 模式时，GPTW Agent 会在生成文件访问 URL 后把地址替换成该 CDN 地址；该变量不会改变默认下载模式。上传仍走 GPTW Agent 后端代理，不会使用 CDN。
