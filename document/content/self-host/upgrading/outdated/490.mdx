---
title: V4.9.0(包含升级脚本)
description: GPTW Agent V4.9.0 更新说明
---
...
- 更新 GPTW Agent 镜像 tag: v4.9.0
- 更新 GPTW Agent 商业版镜像 tag: v4.9.0
- Sandbox 镜像，可以不更新
- 更新 PG 容器为 v0.8.0-pg15, 可以查看[最新的 yml](https://gptw.top)
...
参考[最新的 yml](https://gptw.top) 文件。里面已移除 OneAPI 并添加了 AIProxy 配置。包含一个服务和一个 PgSQL 数据库。将 `aiproxy` 的配置 `追加` 到 OneAPI 的配置后面（先不要删除 OneAPI，有一个初始化会自动同步 OneAPI 的配置）
...
`docker-compose down` 停止服务，然后 `docker-compose up -d` 启动服务，此时会追加 `aiproxy` 服务，并修改 GPTW Agent 的配置。
...
#### 5. 进入 GPTW Agent 检查 `AI Proxy` 服务是否正常启动。
...
### 4. 运行 GPTW Agent 升级脚本

从任意终端，发起 1 个 HTTP 请求。其中 `{{rootkey}}` 替换成环境变量里的 `rootkey`；`{{host}}` 替换成 **GPTW Agent 域名**。
