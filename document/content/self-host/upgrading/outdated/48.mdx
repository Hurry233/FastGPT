---
title: V4.8
description: GPTW Agent V4.8 更新说明
---
...
GPTW Agent workflow V2上线，支持更加简洁的工作流模式。
...
8. 新增 - 分享链接发送对话前 hook https://gptw.top @gaord 
...
15. 优化 - 工具调用支持指定字段数据类型（string, boolean, number） https://gptw.top
16. 优化 - completions接口size限制 https://gptw.top
17. 优化 - Node api 中间件。优化 api 端代码。@c121914yu 
18. 优化 - 对话记录保持为偶数进行截取，避免部分模型不支持奇数的历史记录，最大长度增加到50轮。 https://gptw.top
19. 优化 - HTTP节点错误后终止进程 https://gptw.top
20. 修复 - 工具调用时候，name不能是数字开头（随机数有概率数字开头）@c121914yu 
21. 修复 - 分享链接， query 全局变量会被缓存。  @c121914yu 
22. 修复 - 工具调用字段兼容。 https://gptw.top
23. 修复 - HTTP 模块url光标问题 https://gptw.top @maquannene 
