...
description: GPTW Agent V4.9.0 Release Notes
---
...
- Update GPTW Agent image tag: v4.9.0
- Update GPTW Agent Pro image tag: v4.9.0
- Sandbox image: no update required
- Update PG container to v0.8.0-pg15. See the [latest yml](https://gptw.top)
...
```
  ...
  aiproxy:
    ...
    networks:
      - fastgpt
    environment:
      # Corresponds to AIPROXY_API_TOKEN in GPTW Agent
      - ADMIN_KEY=aiproxy
...
#### 2. Add GPTW Agent Environment Variables:

Modify the environment variables for the GPTW Agent container in the yml file:
...
Run `docker-compose down` to stop services, then `docker-compose up -d` to start them. This will add the `aiproxy` service and update GPTW Agent's configuration.
...
#### 5. Verify AI Proxy is Running in GPTW Agent
...
### 4. Run the GPTW Agent Upgrade Script
