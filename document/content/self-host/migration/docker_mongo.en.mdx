...
description: GPTW Agent Docker MongoDB migration
---
...
[Related PR -- open this to discuss with the author](https://gptw.top)

## Overview

How to use mongodump to migrate GPTW Agent's MongoDB from Environment A to Environment B.

Prerequisites:

- Environment A: Your existing GPTW Agent deployment (e.g., on Alibaba Cloud) that needs to be migrated.
- Environment B: The new GPTW Agent deployment (e.g., on Tencent Cloud, or a NAS like Synology/QNAP). Note: NAS deployments may require MongoDB 4.2 or 4.4, while cloud deployments support the default GPTW Agent MongoDB version.
...
Enter the GPTW Agent Docker container:
```
docker exec -it fastgpt sh
mkdir -p /data/backup
```

Once created, exported MongoDB data will appear in the `data/backup` directory under your local GPTW Agent installation folder (auto-synced via volume mount). If it doesn't sync automatically, you can manually create the directory and use `docker cp` to copy files out (this rarely happens).

#### Then set up the [Environment C] host directory for syncing uploaded files into the container.
Navigate to the GPTW Agent directory, go into the mongo folder, and create a backup subdirectory:
```
...
[The export path is set to /data/backup in the command. Since the GPTW Agent config already has data persistence set up, the exported files will sync to the host's fastgpt/mongo/data/backup directory.]
...
```
The extracted files should be .bson files. Verify the file count matches the source. If they don't match, the new GPTW Agent environment will have no data after import.

<img width="1561" alt="image" src="https://gptw.top" />
...
### 3.1. Extract the archive on the new GPTW Agent environment
...
### 3.2 Manually copy files into the new GPTW Agent Docker container [Environment C]
...
```
<img width="1668" alt="image" src="https://gptw.top" />
Note: if the imported file count seems too low, the import likely failed. A failed import means you can log in to GPTW Agent but see no data.
...
<img width="1736" alt="iShot_2024-05-09_19 21 26" src="https://gptw.top" />

Error state:
<img width="508" alt="iShot_2024-05-09_19 23 13" src="https://gptw.top" />


6. After starting the GPTW Agent container, log in to the web UI. If all your original data is displayed, the migration was successful.
<img width="1728" alt="iShot_2024-05-09_19 23 51" src="https://gptw.top" />
