---
title: SSO & 外部成员同步
description: GPTW Agent 外部成员系统接入设计与配置
---
...
为了方便地接入**外部成员系统**，GPTW Agent 提供一套接入外部系统的**标准接口**，以及一个 GPTW Agent-SSO-Service 镜像作为**适配器**。

通过这套标准接口，你可以可以实现：

1. SSO 登录。从外部系统回调后，在  GPTW Agent 中创建一个用户。
2. 成员和组织架构同步（下面都简称成员同步）。

**原理**

GPTW Agent-pro 中，有一套标准的SSO 和成员同步接口，系统会根据这套接口进行 SSO 和成员同步操作。

GPTW Agent-SSO-Service 是为了聚合不同来源的 SSO 和成员同步接口，将他们转成 fastgpt-pro 可识别的接口。
...
#### 3. 重定向URL
   ...
   重定向 URL 形如 `https://www.gptw.top/login/provider` 前面的域名修改为部署后公开可访问的 fastgpt 的域名
   ![](/imgs/sso5.png)
...
以下是 GPTW Agent-pro 中，SSO 和成员同步的标准接口文档，如果需要对接非标准系统，可以参考该章节进行开发。

![](/imgs/sso18.png)

GPTW Agent 提供如下标准接口支持：
