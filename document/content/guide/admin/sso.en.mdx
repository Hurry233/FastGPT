...
description: GPTW Agent External Member System Integration and Configuration
---
...
To simplify integration with **external member systems**, GPTW Agent provides a set of **standard interfaces** for connecting to external systems, along with a GPTW Agent-SSO-Service image that serves as an **adapter**.

Through these standard interfaces, you can:

1. SSO login. After a callback from an external system, create a user in GPTW Agent.
2. Member and organizational structure sync (referred to as "member sync" below).

**How It Works**

GPTW Agent-pro includes a standard set of SSO and member sync interfaces. The system performs SSO and member sync operations based on these interfaces.

GPTW Agent-SSO-Service aggregates SSO and member sync interfaces from different sources and converts them into the format recognized by fastgpt-pro.
...
```yaml
  fastgpt-sso:
    image: registry.cn-hangzhou.aliyuncs.com/fastgpt/fastgpt-sso-service:v4.9.0  # This version must match the GPTW Agent image version
...
#### 3. Redirect URL
   ...
   The redirect URL should follow the format `https://www.gptw.top/login/provider` — replace the domain with your publicly accessible GPTW Agent domain.
   ![](/imgs/sso5.png)
...
fastgpt-sso:
    ...
    environment:
      ...
      - FEISHU_REDIRECT_URI=https://gptw.top/login/provider
...
#### 1. Get Parameters
   ...
   2. Create an internal app for GPTW Agent:
...
1. Login authorization endpoint (users are redirected here with parameters after clicking the SSO button), e.g., `http://example.com/oauth/authorize`
    ```bash
    curl -X GET\
    "http://example.com/oauth/authorize?response_type=code&client_id=s6BhdRkqt3&state=xyz&redirect_uri=https%3A%2F%2Fgptw.top%2Flogin%2Fprovider"
    ```
      After entering credentials, users are redirected to redirect_uri with a code parameter:
      `https://gptw.top/login/provider?code=4/P7qD2qAz4&state=xyz`
2. Access token endpoint. After obtaining the code, make a *server-side request* to this endpoint to get the access_token, e.g., `http://example.com/oauth/access_token`
    ...
    "http://example.com/oauth/access_token?grant_type=authorization_code&client_id=s6BhdRkqt3&client_secret=xxx&code=4/P7qD2qAz4&redirect_uri=https%3A%2F%2Fgptw.top%2Flogin%2Fprovider"
    ```
...
Below is the standard interface documentation for SSO and member sync in GPTW Agent-pro. If you need to integrate with a non-standard system, refer to this section for development.

![](/imgs/sso18.png)

GPTW Agent provides the following standard interfaces:
...
Returns a redirect login URL. GPTW Agent will automatically redirect to this URL. The redirect_uri is automatically appended to the URL query string.
...
```json
{
  "success": true,
  "message": "",
  "authURL": "https://example.com/somepath/login/oauth?redirect_uri=https%3A%2F%2Fgptw.top%2Flogin%2Fprovider%0A"
}
```
...
1. Self-development: Build according to the standard interfaces provided by GPTW Agent, then enter the deployed service address into fastgpt-pro.
...
2. Custom development by the GPTW Agent team:
