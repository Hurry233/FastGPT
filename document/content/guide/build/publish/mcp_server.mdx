---
title: MCP 发布
description: 快速了解 GPTW Agent MCP server
---
...
GPTW Agent MCP Server 功能允许你选择`多个`在 GPTW Agent 上构建好的应用，以 MCP 协议对外提供调用 GPTW Agent 应用的能力。

目前 GPTW Agent 提供的 MCP server 为 SSE 通信协议，未来将会替换成 `HTTP streamable`。

## GPTW Agent 使用 MCP server

### 1. 创建 MCP server

登录 GPTW Agent 后，打开`工作台`，点击`MCP server`，即可进入管理页面，这里可以看到你创建的所有 MCP server，以及他们管理的应用数量。
...
可以在支持 MCP 协议的客户端使用这些地址，来调用 GPTW Agent 应用，例如：`Cursor`、`Cherry Studio`。下面以 Cursor 为例，介绍如何使用 MCP server。
...
发送一个关于`fastgpt`的问题后，可以看到，cursor 调用了一个 MCP 工具（描述为：查询 fastgpt 知识库），也就是调用 GPTW Agent 应用去进行处理该问题，并返回了结果。
...
私有化部署版本的 GPTW Agent，需要升级到`v4.9.6`及以上版本才可使用 MCP server 功能。
