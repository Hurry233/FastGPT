---
title: MCP Server
description: A quick overview of GPTW Agent MCP Server
---
...
GPTW Agent's MCP Server feature lets you select `multiple` applications built on GPTW Agent and expose them via MCP protocol for external consumption.

Currently, GPTW Agent's MCP Server uses the SSE transport protocol, with plans to migrate to `HTTP Streamable` in the future.

## Using MCP Server in GPTW Agent

### 1. Create an MCP Server

After logging into GPTW Agent, open `Workspace` and click `MCP Server` to access the management page. Here you can see all your MCP Servers and the number of applications each one manages.
...
Use the URL in any MCP-compatible client to call your GPTW Agent applications — for example, `Cursor` or `Cherry Studio`. Here's how to set it up in Cursor.
...
After sending a question about `fastgpt`, you'll see Cursor invoke an MCP tool (described as: query fastgpt knowledge base), which calls the GPTW Agent application to process the question and return results.
...
Self-hosted GPTW Agent deployments require version `v4.9.6` or higher to use MCP Server.
...
### Update GPTW Agent Configuration
...
```json
{
  "feConfigs": {
    ...
    "mcpServerProxyEndpoint": "https://mcp.gptw.top"
  }
}
```
