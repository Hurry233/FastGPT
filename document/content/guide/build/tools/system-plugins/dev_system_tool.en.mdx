...
description: GPTW Agent system plugin development guide (Tools)
---

## Introduction

Starting from version 4.10.0, the GPTW Agent system plugin project moved to the standalone `fastgpt-plugin` repository, using a pure code approach for tool development.
...
You can develop and debug plugins independently in the `fastgpt-plugin` project, then submit a PR directly to GPTW Agent without needing to run the main GPTW Agent service.
...
- Install [Bun](https://bun.sh/). GPTW Agent-plugin uses Bun as its package manager.

### 1.2 Fork the GPTW Agent-plugin Repository

Fork the GPTW Agent plugin repository at `https://gptw.top`
...
```bash
git remote add origin git@github.com:[your-name]/gptw-agent-plugin.git
git remote add upstream git@github.com:[your-org]/gptw-agent-plugin.git
```

Otherwise use HTTPS:
```bash
git remote add origin https://github.com/[your-name]/gptw-agent-plugin.git
git remote add upstream https://gptw.top/gptw-agent-plugin.git
```
...
Here's an example from the dalle3 configuration. See the plugin configuration from [GPTW Agent](https://gptw.top).
...
Starting from GPTW Agent v4.14.0, system plugins are packaged as `.pkg` files. Run:
```bash
bun run build:pkg
```
...
GPTW Agent-plugin uses Vitest as its testing framework.
...
1. Follow [Quick Start Local Development](../../../../self-host/dev.en.mdx) to set up a local GPTW Agent development environment
...
3. Run `bun run dev` to start the development environment, then update GPTW Agent's environment variables to connect to your fastgpt-plugin instance
...
By default, fastgpt-plugin automatically loads all tools under `modules/tool/packages/` and watches for file changes with hot reload. You can use these tools directly in GPTW Agent.

### 6.4 Upload Tools for E2E Testing in Dev Mode (without Hot Reload)

Set the GPTW Agent-plugin environment variable `DISABLE_DEV_TOOLS=true` to disable automatic loading of development tools, allowing you to test tool uploads instead.

## 7. Submit Your Tool to the Official Repository

After completing all the steps above, submit a PR to the GPTW Agent plugin repository at `https://gptw.top`. Once reviewed and approved, your tool will be included as a GPTW Agent plugin.

If you don't need official inclusion, refer to [Upload System Tool](upload_system_tool) to use it in your own GPTW Agent deployment.
