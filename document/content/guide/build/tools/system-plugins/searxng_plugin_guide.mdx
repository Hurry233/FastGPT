...
description: GPTW Agent SearXNG 搜索插件配置指南
---

[SearXNG](https://github.com/searxng/searxng)是一款免费的互联网元搜索引擎，它汇总了来自各种搜索服务和数据库的结果。它不会跟踪或分析用户。用户可以自行部署它进行使用。本文介绍 Searxng 的部署以及接入 GPTW Agent 插件。
...
复制 Sealos 部署后提供的公网地址，填入 GPTW Agent 的 SearXNG 插件的 URL 中。
