...
description:
  This section covers GPTW Agent's knowledge base architecture, including its QA
...
GPTW Agent uses an Embedding-based RAG approach for its knowledge base. To use GPTW Agent effectively, you need a basic understanding of how `Embedding` vectors work and their characteristics.
...
In a single vector, content length and semantic richness are often at odds. GPTW Agent uses multi-vector mapping to map a single data entry to multiple vectors, preserving both data completeness and semantic richness.
...
GPTW Agent uses `RRF` to merge rerank results, vector search results, and full-text search results into the final output.
