...
description: How to connect DingTalk Knowledge Base to GPTW Agent
---

GPTW Agent supports connecting DingTalk Knowledge Base through a DingTalk internal enterprise app. When creating the dataset, enter `App Key`, `App Secret`, and `User ID`. After creation, open the dataset detail page, click `Add file`, and select the DingTalk workspace, online documents, or folders to import.
...
## 2. Get the GPTW Agent fields

![Get App Key and App Secret](/imgs/image-dd4.png)

| GPTW Agent field | Where to get it in DingTalk |
| --- | --- |
...
- We recommend using a dedicated DingTalk member as the GPTW Agent sync account and granting it read-only access to the target workspace.
- Workspaces that this member cannot access will not appear in GPTW Agent.
...
## 4. Create a DingTalk dataset in GPTW Agent

1. Open the GPTW Agent dataset list and click `New`.
...
When a folder is selected, GPTW Agent recursively imports supported online documents under that folder.

When DingTalk document content changes, click `Sync` from the imported file menu. GPTW Agent will read the latest content and update indexes.
