---
title: 钉钉知识库
description: GPTW Agent 钉钉知识库功能介绍和使用方式
---
...
GPTW Agent 支持通过钉钉企业内部应用接入钉钉知识库。创建时只需要填写 `App Key`、`App Secret`、`User ID`，创建完成后进入知识库详情页点击`添加文件`，再选择要导入的钉钉知识库、在线文档或文件夹。
...
## 2. 获取 GPTW Agent 要填写的参数

![获取 App Key 和 App Secret](/imgs/image-dd4.png)

| GPTW Agent 字段 | 钉钉里去哪里拿 |
| --- | --- |
...
- 建议使用一个专门的钉钉成员作为 GPTW Agent 同步账号，并给它目标知识库的只读权限。
- 该成员没有权限访问的钉钉知识库，不会出现在 GPTW Agent 的添加文件列表里。
...
## 4. 在 GPTW Agent 中创建钉钉知识库

1. 进入 GPTW Agent 知识库列表，点击`新建`。
...
选择文件夹时，GPTW Agent 会递归导入该文件夹下支持的在线文档。

钉钉文档内容更新后，可在已导入文件的更多菜单中点击`同步`，GPTW Agent 会重新读取最新正文并更新索引。
